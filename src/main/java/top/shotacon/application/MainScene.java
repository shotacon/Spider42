...
import top.shotacon.application.spider.XVideos;
...
public class MainScene implements Initializable {
    ...
    private static final String xvideos = "xvideos";
...
//    private TextArea textArea;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        List<String> list = Arrays.asList(youtube, pornhub, xvideos);
...
//        tableView.setEditable(true);
    ...
    public void onClickButtonClick(ActionEvent event) {
        ...
        if (siteTypeList.getValue().equals(xvideos)) {
            try {
                ThreadUtil.executorService.submit(new Task<Boolean>() {
                    @Override
                    protected Boolean call() throws Exception {
                        return dataList.addAll(XVideos.doSpider(text));
                    }
                });
            } catch (Exception e) {
                dataList.add(new VideoInfo(TipType.ERROR.getName(), e.getMessage()));
            }
        } else if (siteTypeList.getValue().equals(pornhub)) {
            try {
                ThreadUtil.executorService.submit(new Task<Boolean>() {
                    @Override
                    protected Boolean call() throws Exception {
